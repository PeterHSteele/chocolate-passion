 ...
 * @version   2.6.1 for parent theme Chocolate Passion for publication on WordPress.org
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			// Load class strings.
			$this->strings = array(
				'page_title'                      => __( 'Install Required Plugins', 'chocolate-passion' ),
				'menu_title'                      => __( 'Install Plugins', 'chocolate-passion' ),
				/* translators: %s: plugin name. */
				'installing'                      => __( 'Installing Plugin: %s', 'chocolate-passion' ),
				/* translators: %s: plugin name. */
				'updating'                        => __( 'Updating Plugin: %s', 'chocolate-passion' ),
				'oops'                            => __( 'Something went wrong with the plugin API.', 'chocolate-passion' ),
				...
				'return'                          => __( 'Return to Required Plugins Installer', 'chocolate-passion' ),
				'dashboard'                       => __( 'Return to the Dashboard', 'chocolate-passion' ),
				'plugin_activated'                => __( 'Plugin activated successfully.', 'chocolate-passion' ),
				'activated_successfully'          => __( 'The following plugin was activated successfully:', 'chocolate-passion' ),
				/* translators: 1: plugin name. */
				'plugin_already_active'           => __( 'No action taken. Plugin %1$s was already active.', 'chocolate-passion' ),
				/* translators: 1: plugin name. */
				'plugin_needs_higher_version'     => __( 'Plugin not activated. A higher version of %s is needed for this theme. Please update the plugin.', 'chocolate-passion' ),
				/* translators: 1: dashboard link. */
				'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'chocolate-passion' ),
				'dismiss'                         => __( 'Dismiss this notice', 'chocolate-passion' ),
				'notice_cannot_install_activate'  => __( 'There are one or more required or recommended plugins to install, update or activate.', 'chocolate-passion' ),
			...
			if ( ! empty( $desired_slug ) ) {
				...
				if ( ! empty( $subdir_name ) && $subdir_name !== $desired_slug ) {
					...
					if ( true === $GLOBALS['wp_filesystem']->move( $from_path, $to_path ) ) {
						...
						return new WP_Error( 'rename_failed', esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'chocolate-passion' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'chocolate-passion' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
					}
				} elseif ( empty( $subdir_name ) ) {
					return new WP_Error( 'packaged_wrong', esc_html__( 'The remote plugin package consists of more than one file, but the files are not packaged in a folder.', 'chocolate-passion' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'chocolate-passion' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
				}
			}
			...
			if ( ! empty( $message ) || $total_required_action_count > 0 ) {
				...
				if ( ! current_user_can( 'activate_plugins' ) && ! current_user_can( 'install_plugins' ) && ! current_user_can( 'update_plugins' ) ) {
					...
					foreach ( $message as $type => $plugin_group ) {
						...
						$imploded       = empty( $linked_plugins ) ? $last_plugin : ( implode( ', ', $linked_plugins ) . ' ' . esc_html_x( 'and', 'plugin A *and* plugin B', 'chocolate-passion' ) . ' ' . $last_plugin );

						$rendered .= sprintf(
							$line_template,
							sprintf(
								translate_nooped_plural( $this->strings[ $type ], $count, 'chocolate-passion' ),
								$imploded,
								$count
							)
						);
			...
			echo '<p style="float: right; padding: 0em 1.5em 0.5em 0;"><strong><small>',
				esc_html(
					sprintf(
						...
						__( 'TGMPA v%s', 'chocolate-passion' ),
						self::TGMPA_VERSION
					)
				),
				'</small></strong></p>';
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_plugin_advise_type_text( $required ) {
			if ( true === $required ) {
				return __( 'Required', 'chocolate-passion' );
			}

			return __( 'Recommended', 'chocolate-passion' );
		}
		...
		protected function get_plugin_source_type_text( $type ) {
			...
			switch ( $type ) {
				case 'repo':
					$string = __( 'WordPress Repository', 'chocolate-passion' );
					break;
				case 'external':
					$string = __( 'External Source', 'chocolate-passion' );
					break;
				case 'bundled':
					$string = __( 'Pre-Packaged', 'chocolate-passion' );
					break;
			}
		...
		protected function get_plugin_status_text( $slug ) {
			...
			return sprintf(
				...
				_x( '%1$s, %2$s', 'Install/Update Status', 'chocolate-passion' ),
				$install_status,
				$update_status
			);
		}
			...
			foreach ( $this->view_totals as $type => $count ) {
				...
				switch ( $type ) {
					case 'all':
						...
						$text = _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $count, 'plugins', 'chocolate-passion' );
						break;
					case 'install':
						/* translators: 1: number of plugins. */
						$text = _n( 'To Install <span class="count">(%s)</span>', 'To Install <span class="count">(%s)</span>', $count, 'chocolate-passion' );
						break;
					case 'update':
						/* translators: 1: number of plugins. */
						$text = _n( 'Update Available <span class="count">(%s)</span>', 'Update Available <span class="count">(%s)</span>', $count, 'chocolate-passion' );
						break;
					case 'activate':
						/* translators: 1: number of plugins. */
						$text = _n( 'To Activate <span class="count">(%s)</span>', 'To Activate <span class="count">(%s)</span>', $count, 'chocolate-passion' );
						break;
					default:
						$text = '';
						break;
		...
		public function column_version( $item ) {
			...
			if ( $this->chocolate_passion->is_plugin_installed( $item['slug'] ) ) {
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . __( 'Installed version:', 'chocolate-passion' ) . '</p>',
					$color,
					$installed
				);
			}
			...
			if ( ! empty( $item['minimum_version'] ) ) {
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;">%1$s</span>' . __( 'Minimum required version:', 'chocolate-passion' ) . '</p>',
			...
			if ( ! empty( $item['available_version'] ) ) {
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . __( 'Available version:', 'chocolate-passion' ) . '</p>',
					$color,
		...
		public function no_items() {
			echo esc_html__( 'No plugins to install, update or activate.', 'chocolate-passion' ) . ' <a href="' . esc_url( self_admin_url() ) . '"> ' . esc_html__( 'Return to the Dashboard', 'chocolate-passion' ) . '</a>';
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}
		...
		public function get_columns() {
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'chocolate-passion' ),
				'source' => __( 'Source', 'chocolate-passion' ),
				'type'   => __( 'Type', 'chocolate-passion' ),
			);

			if ( 'all' === $this->view_context || 'update' === $this->view_context ) {
				$columns['version'] = __( 'Version', 'chocolate-passion' );
				...
				<tr class="plugin-update-tr">
					<td colspan="', absint( $this->get_column_count() ), '" class="plugin-update colspanchange">
						<div class="update-message">',
							esc_html__( 'Upgrade message from the plugin author:', 'chocolate-passion' ),
		...
		public function process_bulk_actions() {
			...
			if ( 'chocolate_passion-bulk-install' === $this->current_action() || 'chocolate_passion-bulk-update' === $this->current_action() ) {
				...
				if ( empty( $_POST['plugin'] ) ) {
					if ( 'install' === $install_type ) {
						$message = __( 'No plugins were selected to be installed. No action taken.', 'chocolate-passion' );
					} else {
						$message = __( 'No plugins were selected to be updated. No action taken.', 'chocolate-passion' );
					}
				...
				if ( empty( $plugins_to_install ) ) {
					if ( 'install' === $install_type ) {
						$message = __( 'No plugins are available to be installed at this time.', 'chocolate-passion' );
					} else {
						$message = __( 'No plugins are available to be updated at this time.', 'chocolate-passion' );
					}
			...
			if ( 'chocolate_passion-bulk-activate' === $this->current_action() ) {
				...
				if ( empty( $_POST['plugin'] ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins were selected to be activated. No action taken.', 'chocolate-passion' ), '</p></div>';
				...
				if ( empty( $plugins_to_activate ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be activated at this time.', 'chocolate-passion' ), '</p></div>';
			...
			if ( ! class_exists( 'TGMPA_Bulk_Installer' ) ) {
				...
				class TGMPA_Bulk_Installer extends Plugin_Upgrader {
					...
					public function activate_strings() {
						$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'chocolate-passion' );
						$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'chocolate-passion' );
					}
				...
				class TGMPA_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function add_strings() {
						if ( 'update' === $this->options['install_type'] ) {
							...
							$this->upgrader->strings['skin_before_update_header'] = __( 'Updating Plugin %1$s (%2$d/%3$d)', 'chocolate-passion' );
						} else {
							/* translators: 1: plugin name, 2: error message. */
							$this->upgrader->strings['skin_update_failed_error'] = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'chocolate-passion' );
							/* translators: 1: plugin name. */
							...
							if ( $this->chocolate_passion->is_automatic ) {
								...
								/* translators: 1: plugin name. */
								$this->upgrader->strings['skin_update_successful'] = __( '%1$s installed and activated successfully.', 'chocolate-passion' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'chocolate-passion' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'chocolate-passion' ) . '</span>.</a>';
								$this->upgrader->strings['skin_upgrade_end']       = __( 'All installations and activations have been completed.', 'chocolate-passion' );
								/* translators: 1: plugin name, 2: action number 3: total number of actions. */
								$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'chocolate-passion' );
							} else {
								// Default installation strings.
								$this->upgrader->strings['skin_upgrade_start'] = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'chocolate-passion' );
								/* translators: 1: plugin name. */
								$this->upgrader->strings['skin_update_successful'] = esc_html__( '%1$s installed successfully.', 'chocolate-passion' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'chocolate-passion' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'chocolate-passion' ) . '</span>.</a>';
								$this->upgrader->strings['skin_upgrade_end']       = __( 'All installations have been completed.', 'chocolate-passion' );
								/* translators: 1: plugin name, 2: action number 3: total number of actions. */
								$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'chocolate-passion' );
							}
						}
					}
